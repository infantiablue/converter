...
def respond_popular(sender, country='US'):
    videos = json.loads(get_popular_video_youtube(limit=5, random_videos=True, country=country_code))
    list_video = []
    for video in videos:
        el = {
                'title': video['title'],
...
def respond_download(sender, message):
    ...
    if not os.path.exists(LOCK_FILE_PATH):
        ...
        result = json.loads(download([message], audio_quality='192', action_from='fb_bot'))
        ...
        if result['status']:
            ...
            url_to_download = root_url + '/file/'+ result['data']['path'] + result['data']['extenstion']
            payload = {
                'message':{
                    'attachment':{
                        'type':'template',
        ...
        send_message(sender, 'You have requested to convert from this url {}, I can only handle one for now. Please wait for your previous request finished ⏳'.format(f_name['url']))
            ...
            if is_user_message(x):           
                if x['message'] and x['message']['text']:
                    ...
                    if 'greetings' in nlp_entities:
                        ...
                        send_message(sender_id, 'Hi ' + fb_user['first_name'] + ', may I help you 🤖')
                        ...
                        kw = text.split('search')[-1].split('find')[-1].split('get me')[-1]
                        respond_search_youtube(sender_id, kw)                         
                    elif 'url' in nlp_entities:
                        white_list = ['youtube.com', 'youtu.be']
                        if len(nlp_entities['url']) > 1:
                            send_message(sender_id, 'Sorry, I can only handle 1 link at once for now 😔')
                        else:
                            if nlp_entities['url'][0]['domain'] in white_list:
                                # check if the msg was cached
                                if not tmpl_cache.has_key(msg_id):
                                    def cache_respond(): respond_download(sender_id, nlp_entities['url'][0]['value'])
                                    tmpl_cache.get(key=msg_id, createfunc=cache_respond)
                                else:
                                    print('Cached')
                            else:
                                send_message(sender_id, 'At the moment, I can only process video from YouTube 😔')                        
                    elif text.lstrip().rstrip() == 'help':
                        send_message(sender_id, 'You may type "popular" or "search [keywords]"')                                               
                    else:
                        send_message(sender_id, 'You may type "help" for more instructions.')
            elif 'postback' in x:
                if not tmpl_cache.has_key(str(x['timestamp'])):
                    def cache_respond(): respond_download(sender_id, x['postback']['payload'])
                    tmpl_cache.get(key=str(x['timestamp']), createfunc=cache_respond)
                else:
                    print('Cached')                  
            return 'ok'